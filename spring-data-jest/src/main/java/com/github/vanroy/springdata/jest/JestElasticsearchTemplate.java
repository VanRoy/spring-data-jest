...
public class JestElasticsearchTemplate implements ElasticsearchOperations, ApplicationContextAware {
	...
	public <T> FacetedPage<T> queryForPage(SearchQuery query, Class<T> clazz, JestSearchResultMapper mapper) {
	...
	public <T> FacetedPage<T> queryForPage(StringQuery query, Class<T> clazz, JestSearchResultMapper mapper) {
			...
			public void close() {
				try {
					...
					if (!finished && scrollId != null && currentHits != null && currentHits.hasNext()) {
						//TODO(vanroy) Implement clearScroll on Jest
					}
				} finally {
					currentHits = null;
					scrollId = null;
	...
	private long doCount(SearchSourceBuilder searchRequestBuilder, QueryBuilder elasticsearchQuery, FilterBuilder elasticsearchFilter) {
		if (elasticsearchQuery != null) {
			searchRequestBuilder.query(elasticsearchQuery);
		} else {
			searchRequestBuilder.query(QueryBuilders.matchAllQuery());
	...
	public <T> void delete(DeleteQuery deleteQuery, Class<T> clazz) {
		...
		SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder().query(deleteQuery.getQuery());
	...
	public void delete(DeleteQuery deleteQuery) {
		...
		SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder().query(deleteQuery.getQuery());
		DeleteByQuery deleteByQuery = new DeleteByQuery.Builder(searchSourceBuilder.toString())
				.addIndex(deleteQuery.getIndex())
				.addType(deleteQuery.getType())
				.build();
	...
	public <T> Page<T> moreLikeThis(MoreLikeThisQuery query, Class<T> clazz) {
		...
		MoreLikeThisQueryBuilder moreLikeThisQueryBuilder = moreLikeThisQuery()
				.addItem(new MoreLikeThisQueryBuilder.Item(indexName, type, query.getId()));
...
//            aliasAction.setFilter(query.getFilterBuilder());
	...
	public Set<String> queryForAlias(String indexName) {
		...
		JsonObject entries = result.getJsonObject().getAsJsonObject(indexName).getAsJsonObject("aliases");

		Set<String> aliases = new HashSet<String>(entries.entrySet().size());
	...
	private SearchResult doSearch(SearchSourceBuilder searchSourceBuilder, SearchQuery searchQuery) {
		...
		if (CollectionUtils.isNotEmpty(searchQuery.getFacets())) {
			for (FacetRequest facetRequest : searchQuery.getFacets()) {
				...
				searchSourceBuilder.facet(facet);
			}
		}
